...
def generate_data(N):
    ...
    for base_node_id in range(N):
        ...
        weights = torch.Tensor(np.ones((N - 1,))) * (N - 1.0) + 1  # weight of each edge
        g_1 = dgl.graph(edges)
        g_1.edata['weight'] = weights
    ...
    split_idx = int(train_ratio * 2 * N)
    np.random.seed(1900)
    idx = np.arange(N)
    np.random.shuffle(idx)
    idx_aug = []
    for i, x in enumerate(idx):
        idx_aug.append(int(2 * x))
        idx_aug.append(int(2 * x + 1))
    idx = np.array(idx_aug)
    list_of_graphs_train = [list_of_graphs[j] for j in idx[:split_idx]]
    list_of_graphs_test = [list_of_graphs[j] for j in idx[split_idx:]]

    return list_of_graphs_train, list_of_graphs_test
...
def train(model,
    ...
    best_model_performance = [np.inf, np.inf]
    # best_model = model

    performance_train = evaluate(model, torch.Tensor([0.]), list_of_graphs_train)
    ...
    for epoch in range(1, number_of_epochs + 1):
        graph_losses = []
        for i, graph in enumerate(list_of_graphs_train):
            model.train()
            bailout_ratio = model.predict(graph)
        ...
        if epoch % 10 == 0:
            performance_train = evaluate(model, bailout_capital, list_of_graphs_train)
            performance_test = evaluate(model, bailout_capital, list_of_graphs_test)
            if performance_test < best_model_performance[1]:
                best_model_performance[0] = performance_train
                best_model_performance[1] = performance_test
            my_print(
                "Epoch {:05d} | Train {:10.4f} | Test {:10.4f} | Capital "
...
def run_experiment(N,
                   ...
                   seed,
                   learning_rate):
    list_of_graphs_train, list_of_graphs_test = generate_data(N)
    ...
    run_log = train(model, bailout_capital, number_of_epochs, list_of_graphs_train, list_of_graphs_test, seed, learning_rate)
